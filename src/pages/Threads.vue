...
<page-query>
  query thread {
    threads: allThread  (filter: {date: {gte: "2021-10-14"}},sort: [{ by: "date", order: DESC }, { by: "name", order: ASC }]) {
      edges {
        node {
          id
          title: name
...
export default {
  ...
  computed: { 
    ...
    focusedThreads() { 
      return this.$page.threads.edges.filter(thread => {
        
        //  return (thread.node.issueDate == this.$context.date && thread.node.summaryComplete && thread.node.ignoreConversation !== true && thread.node.channelId == '542945453533036544');
         return (thread.node.path == this.$context.date && thread.node.ignoreConversation !== true && thread.node.channelId == '542945453533036544' && thread.node.status !== 'Not Enough Information');
      });
    } 
  }
};
